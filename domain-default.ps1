...
task CreateNuGetPackage -depends Compile {
  ...
  copy-item $src_directory\Domain.IdentityServer3.EntityFramework.nuspec $dist_directory
  ...
  copy-item $output_directory\IdentityServer3.EntityFramework.pdb $dist_directory\lib\net45
	...
	exec { . $nuget_path pack $dist_directory\Domain.IdentityServer3.EntityFramework.nuspec -BasePath $dist_directory -o $dist_directory -version $packageVersion }
}
